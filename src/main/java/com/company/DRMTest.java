 ...
 */
public class DRMTest extends Test {
    ...
    public void init() {
        ...
        try {
            ...
            config.setCertPassword(tokenPassword);
    ...
    // Test 4: Generate and archive a symmetric key
        ...
        log("\n\nList of completed archival requests");
        KeyRequestInfoCollection completeList = keyClient.listRequests("complete", IRequest.SYMKEY_GENERATION_REQUEST);
        if (completeList.getTotal() == 0) {
            log("No requests found");
        } else {
            Iterator<KeyRequestInfo> iter = completeList.getEntries().iterator();
            while (iter.hasNext()) {
                KeyRequestInfo info = iter.next();
                printRequestInfo(info);
            }
